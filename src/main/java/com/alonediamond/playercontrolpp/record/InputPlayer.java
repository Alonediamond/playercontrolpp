 ...
 *
 * NOTE: This class does NOT press keys directly. Key simulation is handled by
 * ClientEventHandler.simulateKeys(), which reads the getForward/getSideways/etc.
 * values and calls mc.options.*Key.setPressed(). Similarly, sprint key handling
 * is delegated to ClientEventHandler.
 */
public class InputPlayer {
    ...
    // Output values read each tick by ClientEventHandler.simulateKeys()
     ...
     *  The actual key pressing is handled by ClientEventHandler. */
    ...
    public void tick(Minecraft client) {
        ...
        if (state == State.MOVING_TO_START) {
            ...
            playYaw = Mth.wrapDegrees(targetYaw);
            playPitch = 0;
            playForward = 1.0f;
            playSideways = 0;
            playJump = false;
        ...
        // Sprint release delay tracking (actual key press handled by ClientEventHandler)
        if (playSprint) {
            sprintOffTicks = 0;
